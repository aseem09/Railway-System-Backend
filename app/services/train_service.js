...
class TrainService {
    ...
    getSearchResults = (searchQuery, result) => {
        ...
        IN (SELECT train_number \
            ...
            IN (?,?) \
            GROUP BY train_number \
            HAVING COUNT(DISTINCT station_name) = 2) AND station_name = ?) as dest on source.train_number = dest.train_number;",
            [searchQuery.start, searchQuery.stop, searchQuery.start, searchQuery.start, searchQuery.stop, searchQuery.stop],
            (err, res) => {
                if (err) {
                    console.log("Error " + err);
                    result(err, null);
    ...
    getTrainFromId = (train_number, result) => {
        sql.query('SELECT * FROM trains WHERE train_number = ?', [train_number], (err, res) => {
            if (err) {
                console.log("Error " + err);
                result(err, null);
                return;
